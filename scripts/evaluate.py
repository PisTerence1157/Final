        ...
        for batch in tqdm(dataloader, desc='Evaluating'):
            ...
            for i in range(len(case_ids)):
                ...
                if keep_largest:
                    ...
                    # Re-apply threshold logic (already binary)
                    metrics = compute_metrics(pred + 0.6, mask, threshold)  # +0.6 to ensure > 0.5
                else:
...
def main():
    ...
    # Evaluate
    print("\nEvaluating...")
    case_results = evaluate_model(model, dataloader, config, device)
